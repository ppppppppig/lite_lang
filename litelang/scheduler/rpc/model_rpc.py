...
from litelang.models.runner_batch import RunnerBatch
import sys
...
class ModelServerRpc(rpyc.Service):
    def __init__(self, **kwargs):
        ...
        self.model_runner_ = Qwen2ModelRunner(
            config.layer_nums,
            kwargs["max_batch_size"],
            config,
            model_path=kwargs["model_path"],
            max_length=kwargs["max_total_length"],
            mem_usage=kwargs["mem_usage"],
            tp_rank=self.tp_rank_,
            world_size=self.world_size_,
            max_input_length=kwargs["max_input_length"],
            use_radix_cache=kwargs["use_radix_cache"],
        )
        self.batchs_ = {}
...
class ModelClientRPC:
    ...
    def _init_model_rpc(self):
        with concurrent.futures.ThreadPoolExecutor() as executor:
            futures = [
                executor.submit(conn.root.init_model) for conn in self.connections_
            ]
    ...
    def _prefill_rpc(self, batch_id):
        with concurrent.futures.ThreadPoolExecutor() as executor:
            futures = [
                executor.submit(conn.root.prefill, batch_id)
                for conn in self.connections_
            ]
    ...
    def _decode_rpc(self, batch_id):
        with concurrent.futures.ThreadPoolExecutor() as executor:
            futures = [
                executor.submit(conn.root.decode, batch_id)
                for conn in self.connections_
            ]
        output_token_ids, req_ids = futures[-1].result()
        output_token_ids, req_ids = obtain(output_token_ids), obtain(req_ids)
    ...
    def _filter_batch_rpc(self, batch_id, req_ids):
        with concurrent.futures.ThreadPoolExecutor() as executor:
            futures = [
                executor.submit(conn.root.filter_batch, batch_id, req_ids)
                for conn in self.connections_
            ]
    ...
    def _merge_batch_rpc(self, batch_id, new_batch_id):
        with concurrent.futures.ThreadPoolExecutor() as executor:
            futures = [
                executor.submit(conn.root.merge_batch, batch_id, new_batch_id)
                for conn in self.connections_
            ]
        is_finish = futures[-1].result()
        return obtain(is_finish)
