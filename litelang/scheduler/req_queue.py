...
class Req:
    ...
    def Add(self, text: str, output_token):
        ...
        if is_eos_token or (
            self.output_length + self.input_length >= self.max_total_length
            and self.output_length >= self.max_output_length
        ):
            self.is_end = True
...
class SchedulerRunnerQue:
    def __init__(
        ...
        busy_scale,
        max_sum_kv_tokens,
        swapped_strategy="LongestFirstStrategy",
    ):
        self.req_mappings = {}
        self.is_end_ = False
    ...
    def update_from_forward(
        self, output_prompts, output_token_ids, req_ids, eos_token_id
    ):
    ...
    def can_decode(self):
        return (
            self.has_been_consume_tokens
            + self._next_forward_need_tokens()
            - self.max_sum_kv_tokens_
        )
    ...
    def try_swapped_reqs(self):
        ...
        if need_more_tokens > 0:
            swapped_reqs = select_swapped_reqs(
                self.req_mappings.values(), self.strategy_, need_more_tokens
            )
...
class ReadyQue:
    ...
    def try_generate_new_batch(self, last_runner_batch):
        ...
        new_runner_batch = SchedulerRunnerQue(
        ...
        while len(self.reqs_) != 0 and now_batch_size < self.max_batch_size_:
            assert (
                self.reqs_[0].status == ReqStatus.READY
                or self.reqs_[0].status == ReqStatus.SWAPPED
            ), "请求状态错误"
