...
class TreeNode:
    ...
    def split_node(self, match_len):
        ...
        self.child.clear()
        new_child = TreeNode(
            self.token_ids[match_len:], self.page_cache_token_idxs[match_len:]
        )
...
class RadixTree:
    ...
    def insert(self, key, value, shared_key=None):
        ...
        parent_node, has_match_len, match_len = self.find_parent_node(
            now_node, key, 0, shared_key
        )
    ...
    def find_parent_node(self, now_node, key, has_match_len, shared_key):
        ...
        if match_len != now_node.child[child_key].token_ids.size(0):
            ...
            return self.find_parent_node(
                now_node.child[child_key],
                key[match_len:],
                has_match_len,
                shared_key[match_len:],
            )

    def match_prefix(self, key):
        now_node = self.root
        parent_node, has_match_len, value_list, match_len = self.find_prefix_token_idxs(
            now_node, key, [], 0
        )
        if match_len > 0:
            seperate_child_node = parent_node.split_node(match_len)
    ...
    def find_prefix_token_idxs(self, now_node, key, value_list, has_match_len):
        ...
        if match_len != child_node.token_ids.size(0):
            ...
            return self.find_prefix_token_idxs(
                child_node, key[match_len:], value_list, has_match_len
            )
...
def display_the_tree_layer_order(root: TreeNode):
    print(f"==========================================")
    print(
        f"token_ids: {root.token_ids}, page_cache_token_ixs: {root.page_cache_token_idxs}"
    )
