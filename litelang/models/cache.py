...
class Cache(ABC):
    ...
    @abstractmethod
    def GetInputLength(self, query_states: torch.Tensor, layer_idx: int):
        raise NotImplementedError("Make sure to implement `update` in a subclass.")
...
class NormalCache(Cache):
    ...
    def update(
        ...
        covert_key_states_list = self._convert_states_to_normal_cache(
            key_states, model_inputs
        )
        covert_value_states_list = self._convert_states_to_normal_cache(
            value_states, model_inputs
        )
        if key_states is not None:
            if self.key_cache[layer_idx] is None:
                ...
                for idx, _ in enumerate(covert_key_states_list):
                    self.key_cache[layer_idx][idx] = torch.cat(
                        [self.key_cache[layer_idx][idx], covert_key_states_list[idx]],
                        dim=0,
                    )
                    self.value_cache[layer_idx][idx] = torch.cat(
                        [
                            self.value_cache[layer_idx][idx],
                            covert_value_states_list[idx],
                        ],
                        dim=0,
                    )
    ...
    def GetInputLength(self, layer_idx: int):
        if self.key_cache[layer_idx] is None:
            return 0
        else:
...
class PageCache:
    def __init__(
        ...
        # 模型相关参数
        assert (
            num_key_value_heads % tp_world_size == 0
        ), "num_key_value_heads_ should be divisible by tp_world_size_"
    ...
    def _init_kv_buffer(self):
        self.kv_cache_ = torch.empty(
            (
                self.num_layers_,
                self.kv_max_size_,
                self.num_key_value_heads_ * 2,
                self.head_dim_,
            ),
    ...
    def _init_index(self):
        self.req_to_tokens_ = torch.empty(
            (self.max_batch_size_, self.max_length_), dtype=torch.int32
        ).cuda()
        self.free_req_ = FreeReq(self.max_batch_size_)

    def _init_size(self):
        self.kv_max_size_ = int(
            (get_available_gpu_memory() * self.memory_usage_) / 2 * (1024**2)
        )
    ...
    def _write_kv_cache(
        ...
        self.kv_cache_[layer_num, token_idxs, : self.num_key_value_heads_] = key_states[
            start : start + new_token_length - length
        ]
        self.kv_cache_[layer_num, token_idxs, self.num_key_value_heads_ :] = (
            value_states[start : start + new_token_length - length]
        )
    ...
    def free_token_idxs(self, token_idxs):
        assert (
            self.physical_free_token_start_ - token_idxs.size(0) >= 0
        ), "page cache internal error"
        self.physical_free_token_[
            self.physical_free_token_start_
            - token_idxs.size(0) : self.physical_free_token_start_
        ] = token_idxs.cpu()
    ...
    def write_prefill_kv_cache(
        self, reqs, b_start_idx, layer_num, key_states, value_states
    ):
