...
def get_no_padding_messages(input_reqs, is_prefill=True):
    ...
    for idx, req in enumerate(input_reqs):
        ...
        if is_prefill:
            ...
            position_ids.extend(
                [i + len(req.shared_tokens) for i in range(len(req.input_tokens))]
            )
    ...
    return (
        ...
        torch.tensor(position_ids).cuda(),
...
class RunnerBatch:
    ...
    def get_post_sample_para(self):
        temperatures = torch.tensor(
            [req.temperature for req in self.request_mapping.values()]
        ).cuda()
        top_p = torch.tensor(
            [req.top_p for req in self.request_mapping.values()]
        ).cuda()
        top_k = torch.tensor(
            [req.top_k for req in self.request_mapping.values()]
        ).cuda()
        do_sample = torch.tensor(
            [req.do_sample for req in self.request_mapping.values()]
        ).cuda()
    ...
    def _free_req_cache(self, req_id):
        ...
        key = torch.tensor(
            req.shared_tokens + req.input_tokens + req.output_tokens, device="cpu"
        )
        token_idxs = self.kv_cache.get_token_index(
            [
                req,
            ]
        )
        ...
        if has_match_len > len(req.shared_tokens):
            self.kv_cache.free_token_idxs(
                token_idxs[len(req.shared_tokens) : has_match_len].cuda()
            )
    ...
    def update(self, new_runner_batch):
        assert (
            self.is_prefill == False and new_runner_batch.is_prefill == False
        ), "only decode batch can be merged"
        new_output_token_ids = []
        self.request_mapping.update(new_runner_batch.request_mapping)
        self.input_tokens = torch.cat(
            (self.input_tokens, new_runner_batch.input_tokens), dim=0
        )  # 这个后续应该没啥用，没事，先留着
        for req in self.request_mapping.values():
            new_output_token_ids.append(req.output_tokens[-1])
        (
