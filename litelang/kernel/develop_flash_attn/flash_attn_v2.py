...
def gqa_context_attention_fwd(
    ...
    for start_n in range(0, block_end_loc, BLOCK_N):
        ...
        # 加载V并累加
        off_v = (
            (cur_batch_start + start_n + offs_n[:, None]) * stride_vbs
...
def _fwd_kernel_with_no_padding_and_kv_cache_and_prompt_cache(
    ...
    block_end_loc = tl.minimum(
        block_start_loc + BLOCK_M + cur_batch_shared_seq_len,
        cur_batch_seq_len + cur_batch_shared_seq_len,
    )
    ...
    for start_n in range(0, block_mask * block_end_loc, BLOCK_N):
        ...
        mask = (offs_m[:, None] + cur_batch_shared_seq_len) >= (
            start_n + offs_n[None, :]
        )
        qk = tl.where(mask, qk * sm_scale, -1.0e8)
        m_ij = tl.maximum(m_i, tl.max(qk, 1))
...
def test_context_attention_with_kv_cache_and_prompt_cache():
    def generate_test_data(q_heads=8, kv_heads=2, d_model=64):
        ...
        b_start_loc_use_prompt = torch.cat(
            [torch.tensor([0, 8], device=device), torch.cumsum(seq_lens, 0)[:-1]]
        )
        return (
            Q,
            K,
    ...
    def standard_attention():
        ...
        for batch in range(len(seq_lens)):
            ...
            M = torch.tril(
                torch.ones(
                    seq_lens[batch], seq_lens[batch], device=Q.device, dtype=Q.dtype
                )
            )
            # 获取当前batch的token映射
            ...
            for q_head in range(Q.size(1)):
                ...
                scores = torch.matmul(q_data, k.transpose(0, 1)) * (
                    1.0 / (Q.size(-1) ** 0.5)
                )
...
def test_triton_attention():
    ...
    while times > 0:
        ...
        assert torch.allclose(
            output[:, :, 1:, :], standard_out[:, :, 1:, :], atol=1e-3
...
def triton_attention_no_pad():
    def generate_gqa_test_data(q_heads=8, kv_heads=2, d_model=64):
        ...
        b_start_loc = torch.cat(
            [torch.tensor([0], device="cuda"), torch.cumsum(seq_lens, 0)[:-1]]
        ).cuda()
        total_tokens = seq_lens.sum()
    ...
    ref_out = standard_attention_no_pad(
        Q, K, V, b_start_loc, seq_len, kv_group_num=4, sm_scale=0.1
    )
    print(f"standard output: {ref_out}")
    print(f"max diff { torch.max(ref_out - triton_out)}")
    # 验证精度一致性
    assert torch.allclose(triton_out, ref_out, atol=1e-2), "GQA分组场景精度不匹配！"
    print("✅ GQA分组逻辑测试通过")
...
def standard_softmax_attention(Q, K, V, padding_mask, sm_scale):
    ...
    for z in range(Q.size(0)):
        for h in range(Q.size(1)):
            ...
            p[z, h, :, :] = torch.where(
                padding_mask[z, None, None, :], p[z, h, :, :], float("-inf")
            )
