...
def rmsnorm_kernel(
    ...
    for i in range(pid, x_size_x, num_pros):
        ...
        x = tl.load(
            x_ptr + i * x_stride_0 + tl.arange(0, BLOCK) * x_stride_1,
            mask=tl.arange(0, BLOCK) < x_size_y,
            other=0,
        ).to(tl.float32)

        gamma = tl.load(
            gamma_ptr + tl.arange(0, BLOCK) * gamma_stride_0,
            mask=tl.arange(0, BLOCK) < gamma_size_x,
            other=0,
        ).to(tl.float32)
        ...
        tl.store(
            out_ptr + i * x_stride_0 + tl.arange(0, BLOCK) * x_stride_1,
            out,
            mask=tl.arange(0, BLOCK) < x_size_y,
        )
...
def test_rmsnorm(M, N, variance_epsilon=1e-5):
    ...
    gamma = torch.normal(
        size=(N,), mean=0.3, std=0.5, device="cuda", dtype=torch.float32
    )
