...
from scheduler.tokenizerzer import Tokenizer
...
def test(model_path, device):
    ...
    for i in range(100):
        output_token, past_key_values = model.forward(
            output_token, mask, past_key_values
        )
        mask = torch.cat([mask, add_mask], dim=-1)
...
def perf_test(model_path, device):
    ...
    mock_prefill_tensor = torch.ones(
        [batch_size, max_req_length], dtype=torch.float16, device="cuda"
    )
