...
class RowMMWeight:
    def __init__(self, weights, proj_name, bias_name, tp_rank, world_size):
        ...
        assert height % world_size == 0, f"row slice, height: {height} % world_size: {world_size} should equal 0"
        single_gpu_height = height // world_size
        self.proj = weights[proj_name][single_gpu_height * tp_rank: single_gpu_height * (tp_rank + 1)].cuda().to(torch.float32)
        self.bias = weights[bias_name][single_gpu_height * tp_rank: single_gpu_height * (tp_rank + 1)].cuda().to(torch.float32) if bias_name in weights else None

class ColMMWeight:
    def __init__(self, weights, proj_name, bias_name, tp_rank, world_size):
        height, width = weights[proj_name].shape
        assert width % world_size == 0, f"col slice, width: {width} % world_size: {world_size} should equal 0"
        single_gpu_width = width // world_size
        self.proj = weights[proj_name][:, single_gpu_width * tp_rank: single_gpu_width * (tp_rank + 1)].cuda().to(torch.float32)
        self.bias = weights[bias_name][single_gpu_width * tp_rank: single_gpu_width * (tp_rank + 1)].cuda().to(torch.float32) if bias_name in weights else None
...
class Qwen2LayerWeight:
    ...
    def _init_qkv(self, weights):
        ...
        self.q_proj = RowMMWeight(weights, q_proj_name, q_bias_name, self.tp_rank_, self.world_size_)
        self.k_proj = RowMMWeight(weights, k_proj_name, k_bias_name, self.tp_rank_, self.world_size_)
        self.v_proj = RowMMWeight(weights, v_proj_name, v_bias_name, self.tp_rank_, self.world_size_)
        self.o_proj = ColMMWeight(weights, o_proj_name, o_bias_name, self.tp_rank_, self.world_size_)
        
    def _init_input_layernorm(self, weights):
        layernorm_weight = f"model.layers.{self.layer_num_}.input_layernorm.weight"
        layernorm_bias = None
        
        self.layernorm = LayerNormWeight(weights, layernorm_weight, layernorm_bias, self.tp_rank_, self.world_size_)
    ...
    def _init_post_attn_layernorm(self, weights):
        ...
        self.post_attn_layernorm = LayerNormWeight(weights, layernorm_weight, layernorm_bias, self.tp_rank_, self.world_size_)
    ...
    def _init_ffn(self, weights):
        ...
        assert intermediate_size % self.world_size_ == 0, "It should be intermediate_size % worldd_size == 0"
        single_gpu_intermediate_size = intermediate_size // self.world_size_
        self.down_proj = weights[down_weight][:, single_gpu_intermediate_size * self.tp_rank_: single_gpu_intermediate_size * (self.tp_rank_ + 1)].cuda().to(torch.float32)
        self.gate_proj = weights[gate_weight][single_gpu_intermediate_size * self.tp_rank_: single_gpu_intermediate_size * (self.tp_rank_ + 1), : ].cuda().to(torch.float32)
        self.up_proj = weights[up_weight][single_gpu_intermediate_size * self.tp_rank_: single_gpu_intermediate_size * (self.tp_rank_ + 1), :].cuda().to(torch.float32)
