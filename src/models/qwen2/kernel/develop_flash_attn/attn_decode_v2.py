...
def _fwd_kernel(
    ...
    for start_n in range(0, cur_batch_seq_len, BLOCK_N):
        ...
        k_ptrs = K + physical_loc[None, :] * stride_kbs + cur_kv_head * stride_kh + offs_d[:, None]
        ...
        # update acc
        v_ptrs = V + physical_loc[:, None] * stride_vbs + cur_kv_head * stride_vh + offs_d[None, :]
...
def test_gqa_decode_attention():
    ...
    req_starts = torch.cumsum(torch.cat([torch.tensor([0], device="cuda"), req_lens[:-1]]), dim=0)
    total_tokens = int(req_lens.sum())
    
    # 生成输入数据 (紧凑存储)
    Q = torch.randn(num_requests, num_q_heads, d_model, device="cuda", dtype=torch.float16)
    K = torch.randn(total_tokens, num_kv_heads, d_model, device="cuda", dtype=torch.float16)
    V = torch.randn(total_tokens, num_kv_heads, d_model, device="cuda", dtype=torch.float16)
