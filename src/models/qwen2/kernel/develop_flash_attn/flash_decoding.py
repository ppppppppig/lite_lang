...
def token_decode_attention_flash_decoding(
    q, req_to_token_indexs, b_req_idx, b_seq_len, max_len_in_batch, q_head_num, head_dim, cache_k, cache_v, out=None, alloc_tensor_func=torch.empty
):
    BLOCK_SEQ = 256
    batch_size = q.size(0)
...
def test_flash_decoding():
    ...
    token_decode_attention_flash_decoding(Q, Req_to_tokens, B_req_idx, b_seq_len, max_len_in_batch, q_head_num, head_dim, K, V, out_flas_decoding)
    ...
    assert torch.allclose(out_flas_decoding, standard_out, atol=1e-2), "Output does not match the standard output."
