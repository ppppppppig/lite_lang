...
class AttnCudaGraph:
    def __init__(self, batch_size, num_heads, seq_len, head_dim, sm_scale):
        ...
        # 创建计算图的输入和输出张量
        self.Q = torch.empty((batch_size, num_heads, seq_len, head_dim), device="cuda", dtype=torch.half)
        self.K = torch.empty((batch_size, num_heads, seq_len, head_dim), device="cuda", dtype=torch.half)
        self.V = torch.empty((batch_size, num_heads, seq_len, head_dim), device="cuda", dtype=torch.half)
        self.output = torch.empty((batch_size, num_heads, seq_len, head_dim), device="cuda", dtype=torch.half)
        torch.cuda.init()
        # 构造 CUDA Graph
    ...
    def _build_cuda_graph(self):
        ...
        try:
            ...
            with torch.cuda.graph(g):
                self.output.copy_(self._compute_attention(self.Q, self.K, self.V, self.sm_scale))
            # 保存捕获到的 CUDA Graph
            self.graph = g
    ...
    def _compute_attention(self, Q, K, V, sm_scale):
        ...
        # Mask: 使用下三角 mask
        M = torch.tril(torch.ones(self.seq_len, self.seq_len, device="cuda", dtype=torch.bool))
