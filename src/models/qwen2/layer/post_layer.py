...
class Qwen2PostLayer:
    ...
    def Forward(self, hidden_states, model_inputs):
        will_compute_hidden_states = self._select_hidden_states(hidden_states, model_inputs)
    ...
    def _PostProcess(self, logits, model_inputs):
        ...
        logits_sort[torch.arange(0, logits_sort.size(-1)).cuda().view(1, 1, -1) >= top_k[:, None, None]] = 0.0
