...
class HttpServerManager:
    ...
    def __init__(self, model_path, max_input_length, max_output_length, max_batch_size, mem_usage, tp, max_reqs, busy_scale, use_radix_cache):
        max_total_length = max_input_length + max_output_length
        config = Qwen2Config(os.path.join(model_path, 'config.json'))
        model_client = ModelClientRPC(tp, max_batch_size, max_input_length, config, model_path, max_total_length, mem_usage, use_radix_cache)
